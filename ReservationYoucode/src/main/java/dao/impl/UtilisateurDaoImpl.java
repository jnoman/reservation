...
public class UtilisateurDaoImpl implements UtilisateurDao {
	...
	public Utilisateur login(UtilisateurConnection user) {
		...
		if(utilisateur != null) {
			if(! BCrypt.checkpw(user.getPassword(), utilisateur.getPassword())) {
				utilisateur = null;
			}
		}
		return utilisateur;
	...
	public List<Utilisateur> getAllApprenant() {
		return currentSession().createQuery("from Utilisateur where active = 1 and role = 'user'", Utilisateur.class).list();
	}
